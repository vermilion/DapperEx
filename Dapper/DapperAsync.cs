...
namespace Dapper
{
    public static partial class SqlMapper
    {
        ...
        {
            return QueryAsync<T>(cnn, typeof(T), new CommandDefinition(sql, (object)param, transaction, commandTimeout, commandType));
        }
        ...
        public static Task<IEnumerable<object>> QueryAsync(this IDbConnection cnn, Type type, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null)
        {
            ...
            return QueryAsync<object>(cnn, type, new CommandDefinition(sql, (object)param, transaction, commandTimeout, commandType));
        }
        ...
        public static Func<IDataReader, object> GetDeserializer(Type type, string splitOn, IDataReader reader, ref int currentPos)
        {
            int splitPoint = GetNextSplit(currentPos, splitOn, reader);

            var deserializer = GetDeserializer(type, reader, splitPoint, currentPos-splitPoint, false);
        ...
        partial class GridReader
        {
            ...
            {
                if (await ((DbDataReader)Reader).NextResultAsync(_cancel).ConfigureAwait(false))
                {
                    _readCount++;
                    GridIndex++;
                    _consumed = false;
            ...
            private async Task<IEnumerable<T>> ReadBufferedAsync<T>(int index, Func<IDataReader, object> deserializer, Identity typedIdentity)
            {
                //try
                //{
                    var reader = (DbDataReader)Reader;
                    var buffer = new List<T>();
                    while (index == GridIndex && await reader.ReadAsync(_cancel).ConfigureAwait(false))
                    {
        ...
        public static async Task<GridReader> QueryMultipleAsync(this IDbConnection cnn, CommandDefinition command)
        {
            ...
            try
            {
                ...
                reader = await cmd.ExecuteReaderAsync(wasClosed ? CommandBehavior.CloseConnection | CommandBehavior.SequentialAccess : CommandBehavior.Default, command.CancellationToken).ConfigureAwait(false);
